...
const SideBar : React.FC<{chats: ChatData[], activeChat: any, logout:Function, user: {id:number, name:string} | null, setActiveChat:Function, onSendPrivateMessage: Function}>
 = ({chats, activeChat,  logout, user, setActiveChat, onSendPrivateMessage}) =>{
    ...
    return(
            <div id="side-bar">
                ...
                <div className="users"
                    onClick={(e)=>{(e.target === props.user) && setActiveChat(null)}}>
                        
                        {chats.map((chat : ChatData)=>{
                            if(chat.name){
                                const lastMessage = chat. messages[chat.messages.length-1];
                                //Fix later
                                const user : any= chat.users.find(({name} : any)=>{
