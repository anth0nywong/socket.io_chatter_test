...
export default function(socket){
    ...
    socket.on(events.PRIVATE_MESSAGE, ({receiver, sender})=>{
        ...
        if(receiver in connectedUsers)
        {
            ...
            const privateChatUserPair : string = receiver+"&"+sender;
            const privateChatUserPair2 : string = sender+"&"+receiver;
            if(privateChatUsers.includes(privateChatUserPair) || privateChatUsers.includes(privateChatUserPair2))
            {
                return false;
            }
            privateChatUserPair.Add(privateChatUserPair);
